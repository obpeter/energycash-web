...
const ParticipantDetailsPaneComponent: FC<ParticipantDetailsPaneProps> = (props) => {

  const {selectedParticipant} = props;
  // const selectedParticipant = useAppSelector(selectedParticipantSelector);

  // const selectedMeter = useAppSelector(selectedMeterById(selectedParticipant.meters[0].meteringPoint))
  // const selectedMeter: Metering | undefined = {} as Metering
  const [selectedMeter, setSelectedMeter] = useState<Metering|undefined>()
  ...
  const dynamicComponent = (componentKey: DynamicComponentKey, participant: EegParticipant) => {

    switch (componentKey) {
      case "memberForm":
        return <MemberFormComponent participant={selectedParticipant} formId={""} onSubmit={(e) => console.log("update", e)}/>
      case "meterForm":
        if (selectedMeter) {
    ...
    <div className={"details-body"}>
      ...
      <div style={{display:"flex", flexDirection: "row"}}>
        <div style={{display:"flex", flexDirection:"column", width: "50%"}}>
          <div className={"details-box"}>
            ...
            <div>
              <IonItem lines="full" className={"eeg-item-box"}>
                ...
                <div>
                  <div className={"detail-header"}>{`Mitglied ist ${selectedParticipant?.role === 'EEG_USER' ? 'kein' : ''} Administrator`}</div>
                </div>
          ...
          <div className={"details-box"}>
            <IonToolbar color="primary"><IonTitle>{formatMeteringPointString(selectedMeter?.meteringPoint)}</IonTitle></IonToolbar>
            <IonItem lines="full" className={"eeg-item-box"} disabled={isMeterPending()}>
              ...
              <IonToggle slot="end" checked={selectedMeter?.status === "ACTIVE"}></IonToggle>
            </IonItem>
            <IonItem button lines="full" className={cn("eeg-item-box", {"selected": activeMenu === "meterForm"})} onClick={() => setActiveMenu("meterForm")}>
            ...
            <div style={{height: "300px", width: "100%"}}>
              <ResponsiveContainer width="90%" height={300}>
                ...
                <BarChart
                  width={500}
                  height={300}
                  data={energySeries.map((e, i) => {return {name: "" + (i + 1), distributed: e.allocated, consumed: e.consumed}})}
                  margin={{
                    top: 5,
                    right: 30,
                    left: 20,
        ...
        <div className="pane-content-details">
          <div className="pane-content-details-content">
            ...
            {dynamicComponent(activeMenu, selectedParticipant)}
